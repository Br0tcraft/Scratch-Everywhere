...
void Parser::loadSprites(const nlohmann::json &json) {
    ...
    for (const auto &target : spritesData) {
        ...
        // Blocks - sammle procedures_call blocks für später
        ...
        if (!target["blocks"].empty()) {
            ...
            for (const auto &[id, data] : target["blocks"].items()) {
                ...
                newBlock->nextBlock = loadBlock(newSprite, nextBlockKey, target["blocks"], 0, 2);
            }
            ...
            for (const std::string &id : procedureCallBlocks) {
                ...
                const auto &prototype = target[data["inputs"]["custom_block"][1]];
                ...
                // Parse argument IDs
                if (data["mutation"].contains("argumentids")) {
                    std::string rawArgumentIds = data["mutation"]["argumentids"];
                    try {
                        nlohmann::json parsedAIds = nlohmann::json::parse(rawArgumentIds);
                        definitionBlock->argumentIDs = parsedAIds.get<std::vector<std::string>>();
                    } catch (const std::exception &e) {
                        Parser::log("\t\t! Failed to parse argumentids: " + std::string(e.what()));
                    }
                }

                // Parse argument defaults
                if (data["mutation"].contains("argumentdefaults")) {
                    ...
                    try {
                        ...
                        for (const auto &item : parsedAD) {
                            if (item.is_string()) {
                                definitionBlock->argumentDefaults.push_back(item.get<std::string>());
                            } else if (item.is_number_integer()) {
                                definitionBlock->argumentDefaults.push_back(std::to_string(item.get<int>()));
                            } else if (item.is_number_float()) {
                                definitionBlock->argumentDefaults.push_back(std::to_string(item.get<double>()));
                            } else {
                                definitionBlock->argumentDefaults.push_back(item.dump());
                            }
                        }
                    } catch (const std::exception &e) {
                        Parser::log("\t\t! Failed to parse argumentdefaults: " + std::string(e.what()));
                    }
                }
            }
        }
...
void Parser::loadAdvancedProjectSettings(const nlohmann::json &json) {
    ...
    try {
        ...
        Parser::log("\tFPS: " + std::to_string(Scratch::FPS));
    } catch (...) {
#ifdef DEBUG
        Parser::log("\t! No framerate property");
#endif
    }

    try {
        Scratch::turbo = config["turbo"].get<bool>();
        Parser::log("\tTurbo: " + std::string(Scratch::turbo ? "Yes" : "No"));
    } catch (...) {
#ifdef DEBUG
        Parser::log("\t! No turbo property");
#endif
    }

    try {
        Scratch::hqpen = config["hq"].get<bool>();
        Parser::log("\tHQ Pen: " + std::string(Scratch::hqpen ? "Yes" : "No"));
    } catch (...) {
#ifdef DEBUG
        Parser::log("\t! No hqpen property");
#endif
    }

    try {
        Scratch::projectWidth = config["width"].get<int>();
        Parser::log("\tWidth: " + std::to_string(Scratch::projectWidth));
    } catch (...) {
#ifdef DEBUG
        Parser::log("\t! No width property");
#endif
    }

    try {
        Scratch::projectHeight = config["height"].get<int>();
        Parser::log("\tHeight: " + std::to_string(Scratch::projectHeight));
    } catch (...) {
#ifdef DEBUG
        Parser::log("\t! No height property");
#endif
    }

    try {
        Scratch::fencing = config["runtimeOptions"]["fencing"].get<bool>();
        Parser::log("\tFencing: " + std::string(Scratch::fencing ? "Yes" : "No"));
    } catch (...) {
#ifdef DEBUG
        Parser::log("\t! No fencing property");
#endif
    }

    try {
        Scratch::miscellaneousLimits = config["runtimeOptions"]["miscLimits"].get<bool>();
        Parser::log("\tMisc Limits: " + std::string(Scratch::miscellaneousLimits ? "Yes" : "No"));
    } catch (...) {
#ifdef DEBUG
        Parser::log("\t! No misc limits property");
#endif
    }
...
#ifdef DEBUG
        Parser::log("\t! No maxClones property");
#endif
    }
    ...
    if (infClones) {
        ...
        Parser::log("\tMax Clones: Unlimited");
    } else {
        Scratch::maxClones = 300;
        Parser::log("\tMax Clones: 300");
    }
}
...
Block *Parser::loadBlock(Sprite *newSprite, const std::string id, const nlohmann::json &blockDatas, Block *parentBlock, int indent) {
    ...
    if (newBlock->opcode == "procedures_call") {
        if (blockData.contains("mutation") && blockData.is_object() &&
            ...
            if (Parser::costumeHatBlock.count(blockData["mutation"]["proccode"]) == 0) Parser::costumeHatBlock[blockData["mutation"]["proccode"]] = new Block();
            newBlock->MyBlockDefinitionID = Parser::costumeHatBlock[blockData["mutation"]["proccode"]];
        }
    }
