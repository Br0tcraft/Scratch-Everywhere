...
SCRATCH_BLOCK(event, touchingobjectmenu) {
    if (block->fields.count("TOUCHINGOBJECTMENU") == 0) return BlockResult();
    *outValue = Value(block->fields["TOUCHINGOBJECTMENU"].value);
    return BlockResult::CONTINUE;
}