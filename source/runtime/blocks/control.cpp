...
SCRATCH_BLOCK(control, if) {
    ...
    Scratch::resetInput(block, "CONDITION");
...
SCRATCH_BLOCK(control, create_clone_of_menu) {
    if (block->fields.count("CLONE_OPTION") == 0) return BlockResult::CONTINUE;
    *outValue = Value(block->fields["CLONE_OPTION"].value);
    return BlockResult::CONTINUE;
}
...
SCRATCH_BLOCK(control, wait) {
    ...
    if (state->completedSteps == 1) {
        if (state->waitTimer.hasElapsed(state->waitDuration)) {
            thread->eraseState(block);
            Scratch::resetInput(block, "DURATION");
            return BlockResult::CONTINUE_IMIDIATELY;
        }
        return BlockResult::REPEAT;
    }
...
SCRATCH_BLOCK(control, repeat) {
    ...
    if (state->repeatTimes < 0) { // start (-1)
        thread->eraseState(block);
        Scratch::resetInput(block, "TIMES");
        return BlockResult::CONTINUE;
    }
