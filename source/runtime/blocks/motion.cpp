...
SCRATCH_BLOCK(motion, goto_menu) {
    if (block->fields.count("TO") == 0) return BlockResult();
    *outValue = Value(block->fields["TO"].value);
    return BlockResult::CONTINUE;
}

SCRATCH_BLOCK(motion, glideto_menu) {
    if (block->fields.count("TO") == 0) return BlockResult();
    *outValue = Value(block->fields["TO"].value);
    return BlockResult::CONTINUE;
}

SCRATCH_BLOCK(motion, pointtowards_menu) {
    if (block->fields.count("TOWARDS") == 0) return BlockResult();
    *outValue = Value(block->fields["TOWARDS"].value);
    return BlockResult::CONTINUE;
}
